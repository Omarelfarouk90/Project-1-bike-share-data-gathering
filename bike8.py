...
month_dic = {1:"January", 2:"february", 3:"March", 4:"April", 5:"May", 6:"June", 7:"all"}
...
def get_filters():
    ...
    """
    print('Good morning ! let us try to analyse som data form the US bikeshare !')
    ...
    while True:      
        ...
        if (ValueError == True):
            city_input = input("\wrong value, please choose an appropriate value\n")
            letter = city_input.lower()
        elif (KeyboardInterrupt == True):
            city_input = input("\nDont try to escape, please choose an appropriate value\n")
            letter = city_input.lower()
        elif ((type(letter) == str) and (letter not in letters_dic)):
            city_input = input("\n not a valid value, please choose an appropriate value\n")
            letter = city_input.lower()
        elif ((type(letter) == int) and (letter not in letters_dic)):
            city_input = input("\n not a valid value, please choose an appropriate value\n")
            letter = city_input.lower()
        else:
            break
    ...
    print("Enter capital 'A' to apply no month filter to the data")
    print("Please enter 1 for January and 6 for June in that order")
    month_input = input("Enter the month you want to filter\n")
    while True:
        if (month_input == 'A' or month_input == 'a' ):
            month = 7
        else:
            month = int(month_input)
            if (month not in month_dic):
                month_input = input("\nInvalid input; Enter the month you want to filter again or  letter 'A' for all \n")
                month = int(month_input)
